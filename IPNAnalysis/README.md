...
| plotting | `--fixed-y` | Use fixed y-axis limits across wells; if spike times are missing, regenerate fixed-y plots from existing `network_results.json` when available |
...
- `--fixed-y` still needs the project-level y-max summary JSON; if `spike_times.npy` is absent, it now attempts to regenerate fixed-y plots from the well’s existing `network_results.json`
