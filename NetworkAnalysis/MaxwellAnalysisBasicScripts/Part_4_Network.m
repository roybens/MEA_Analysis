...
pathFileNetwork =  '/mnt/disk20tb/Organoids/Maxone/RTT_Organoids_T2_02022024_KMB/RTT_Organoids_T2_02022024_KMB/240223/16795/Network/000060/data.raw.h5';
...
% Plotting
plot(networkData.rawMap.map.x, networkData.rawMap.map.y, 'o');
xlabel('X');
ylabel('Y');
...
firingRateNorm2  = (firingRate - min(firingRate))/(max(firingRate)-min(firingRate));
%save('NetworkAct16795.mat',"firingRateNorm")
% Let's plot the Network Activity below the raster. 

subplot(2,1,2);
plot(timeVector,firingRateNorm2,'Color','#135ba3')
xlim([0 round(max(relativeSpikeTimes.time)/4)])
%ylim([0 10])
...
minPeakDist =3.0;
rmsFiringRate = mxw.util.rms(firingRateNorm2);

[peaks, peakIndices] = findpeaks(firingRateNorm2,timeVector,'MinPeakHeight',...
