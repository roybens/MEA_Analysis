...
% Create a uifigure window
    ...
    % Create buttons and other controls in a column on the right side
    columnX = 400;
    spacingY = 40;
...
function processActivityButtonCallback(~, ~)
    ...
    % Handle toggle state
    if rasterToggle.Value
        rasterToggle.Text = 'ON';
        %setting the plot fig
        plotFig = true;
    else
        rasterToggle.Text = 'OFF';
        plotFig = false;
    end
        % Handle toggle state
    if extendedMetricsToggle.Value
        extendedMetricsToggle.Text = 'ON';
        %setting the plot fig
        extMetricsFlag = true;
    else
        extendedMetricsToggle.Text = 'OFF';
        extMetricsFlag = false;
    end
    % Create a struct and store the values
    data = struct();
    data.projectName = projectName;
   ...
   function toggleRasterCallback(src, event)
    % Check the state of the toggle button
    if src.Value
        src.Text = 'ON';  % Update the button text to ON
        disp('Raster Plot is Enabled');
        % Place additional code here to handle the raster plot enabling
    else
        src.Text = 'OFF';  % Update the button text to OFF
        disp('Raster Plot is Disabled');
        % Place additional code here to handle the raster plot disabling
    end
   end
    function extendedMetricsCallback(src, event)
        % Check the state of the toggle button
        if src.Value
            src.Text = 'ON';  % Update the button text to ON
            disp('Extended Metrics is Enabled');
            % Place additional code here to handle the raster plot enabling
        else
            src.Text = 'OFF';  % Update the button text to OFF
            disp('Extended Metrics is is Disabled');
            % Place additional code here to handle the raster plot disabling
        end
...
>>>>>>> a53c63a (Chnges separating plotting from netwrok analysis)
