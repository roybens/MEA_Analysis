...
def sort_recording_list(path_list, save_root, sorter, sorter_params = dict(), clear_files=True, verbose=True, stream_select = None):
    """
...
def spikesort_recordings(continuous_h5_dirs, sortings_dir = default_sorting_dir, allowed_scan_types=["AxonTracking"], stream_select = None):
    ...
    for dir in continuous_h5_dirs.values():
        ...
        if scantype in allowed_scan_types:
            ...
            #sorting_list_by_stream = 
            sorting_list_by_stream = sort_recording_list(h5_file_path, save_root=spikesorting_root, sorter = 'kilosort2_5', sorter_params = sorter_params, verbose = verbose, stream_select = stream_select) 
            sorting_lists_by_scan.append(sorting_list_by_stream)
