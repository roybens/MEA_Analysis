...
class AxonReconstructor:
    ...
    def __init__(self, h5_parent_dirs, **kwargs):
        ...
        #self.use_project_name = kwargs.get('use_project_name', True)
    ...
    def load_recordings(self):
        ...
        for dir in self.h5_parent_dirs:
            ...
            for h5_path in h5_file_paths:
                ...
                project_name = h5_details[0]['projectName']
                ...
                if scan_type in self.sorting_params['allowed_scan_types']:
                    ...
                    if self.reconstructor_id is None: 
                        self.reconstructor_id = f"{date}_{chip_id}_{run_id}" #TODO: This is a hacky way to set the reconstructor_id, need to fix such that there is one reconstructor per recording
    ...
    def spikesort_recordings(self):
        ...
        for rec_key, multirec in self.multirecordings.items():
            ...
            for stream_id, stream in multirec['streams'].items():
                ...
                try:
                    assert self.reconstructor_load_options['load_reconstructor'], 'Load existing sortings option from reconstructor is set to False. Will attempt to load from filepath.'#'Load existing sortings option from reconstructor is set to False. Generating new sorting.'
